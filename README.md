...
**측정 결과 (2026-06-19 · `--demo` 15회, baseline CSV 고정)**

밸런싱이 *목표를 실제로 맞추는지* 처음으로 정량 측정한 결과, **자기 목표 대비 일관되게 너무 쉽다(miscalibrated).**

| 지표 | 관측 | 목표가 맞다면 기대 |
|------|------|--------------------|
| 클리어율 | 13/15 ≈ **87%** | — |
| 무패 클리어(0 losses) | 8/15 ≈ **53%** | — |
| 4보스 전부 통과 | 대부분 | 보스 적 승률 40%면 `0.6⁴ ≈ 13%` |

보스가 "적 승률 40%"로 튜닝됐다면 한 판에 4보스를 다 잡을 확률은 ~13%여야 하는데 실제론 대부분 클리어한다. **클리어가 잘 되는 것 자체가 목표 미달의 신호.** 유력 원인은 위 *실/시뮬 로직 중복* — 튜너는 시뮬 속 플레이어 기준 40%를 맞추지만 실제 플레이어가 시뮬보다 강하게 행동(예: 시뮬은 스킬을 랜덤, 실전은 사용 가능한 첫 스킬)하면 *"튜닝한 난이도 ≠ 실제 난이도"* 가 된다. 검증·수치화·원인 규명은 이슈로 추적한다(달성치 노출 → 다회 MC 리포트 → sim↔real 격차 진단).
